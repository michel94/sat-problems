...
class Formula:
	...
	def __iand__(self, right):
		...
		return self
	def __str__(self):
		return "\n".join([" | ".join([str(v) for v in c.vars]) for c in self.clauses])
...
class Solver:
	...
	def solve(self, formula, verbose=False):
		...
		if verbose:
			print("Formula with", len(d), "variables")
			print("Formula with", len(formula.clauses), "clauses")

		resFilename = "/tmp/out.cnf"
		extraFilename = "/tmp/extra.txt"
		null = open(os.devnull, 'w')
		extra = open(extraFilename, 'w')
